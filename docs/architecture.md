...
| Production session port | Enumerates candidates, captures one opaque virtual-desktop snapshot, creates native tiles, maps pointer/layout events, and attempts foreground activation | `FrigoTab/SessionForm.cs`, `FrigoTab/DesktopSnapshot.cs`, `FrigoTab.Core/ISwitcherSessionPort.cs` |
...
| Desktop backdrop | Captures the composed virtual desktop once with native `BitBlt`, retains the opaque image for the session, and paints it at native size with a black fallback | `FrigoTab/DesktopSnapshot.cs`, `FrigoTab/SessionForm.cs` |
...
## Opaque desktop snapshot model

`SessionForm` remains hidden while `DesktopSnapshot` captures the complete composed virtual desktop with one native `BitBlt`. The snapshot owns the native capture resources and retains the resulting opaque image until the session closes. `OnPaint` uses an exact-size native paint when the client and virtual-desktop bounds match; scaling is only a fallback for an unusual DPI or topology mismatch, and the owner clears to black only when capture or paint is unavailable. This keeps repaint work bounded and avoids repeatedly reading pixels from the desktop or filling the successful path twice.

The background is deliberately static for the lifetime of a session. It represents exactly what was visible when the gesture began, including applications and taskbars, while the individual application previews remain opaque DWM thumbnails layered above it. No per-window background thumbnails, z-order search, or manual reconstruction is performed. A protected, secure, unavailable, or failed capture produces a solid black background and the switcher remains fail-open. Display/DPI topology changes close the session so the next gesture captures a fresh image. Visual fidelity and native capture behavior still require the manual Windows matrix.
